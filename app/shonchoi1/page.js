...
function MemberCard({
  ...
  return (
    <Reorder.Item
      key={item._id}
      value={item}
      dragListener={false} // 👈 কার্ডের যেকোনো স্থানে ড্রাগ ডিজেবল
    ...
    >
      <div className="flex items-center gap-2 bg-amber-500 text-black p-3.5 rounded-2xl shadow-lg hover:bg-amber-400 transition">
        {/* 🔹 শুধুমাত্র এই আইকনটি চেপে ধরলেই ড্রাগ করা যাবে */}
        {isAdmin && (
          <div
            onPointerDown={(e) => dragControls.start(e)} // 👈 আইকনে পয়েন্টার ডাউন হলে ড্রাগ স্টার্ট হবে
        ...
        <Link href={`/shonchoi1/${item._id}`} className="flex-1">
...
export default function Shonchoi() {
  ...
  const fetchMembers = async () => {
    try {
      const res = await fetch("/api/members1")
